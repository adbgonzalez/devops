...
@app.route("/")
def hello():
    return "Ola dende Flask con usuario non root e HEALTHCHECK!"
...
```bash
#!/bin/sh
echo "Iniciando contedor... executando comando: $@"
exec "$@"
...
# Copiar dependencias e instalalas
COPY requirements.txt .
RUN pip install --no-cache-dir -r requirements.txt

# Copiar a app e o script de inicio
...
# Healthcheck chamando á app
HEALTHCHECK --interval=30s --timeout=3s --retries=3 \
  CMD wget -qO- http://127.0.0.1:8000/ >/dev/null 2>&1 || exit 1

# Executamos sempre a través de start.sh
ENTRYPOINT ["./start.sh"]

# O comando que start.sh vai executar para lanzar Flask
CMD ["python", "app.py"]
